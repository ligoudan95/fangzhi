...
func test_progress_mismatch_no_advance() -> void:
	...
	var wrong_type: Dictionary = QuestTracker.add_progress(s, 2, 1, 1, _quests)
	assert_bool(wrong_type.progressed).is_false()
	var wrong_target: Dictionary = QuestTracker.add_progress(s, 1, 99, 1, _quests)
...
func test_prologue_chain_into_chapter_one() -> void:
	var s: Dictionary = QuestTracker.advance_story(QuestTracker.create_state(), _quests).state
	s = QuestTracker.add_progress(s, 1, 1, 1, _quests).state
	var r: Dictionary = QuestTracker.add_progress(s, 2, 0, 1, _quests)
